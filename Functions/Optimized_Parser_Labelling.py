...
def label_CICIDS(pcap_csv, CICIDS_csv, output_file, file_num):
    ...
    df_CICIDS_csv = df_CICIDS_csv[
        ["Timestamp", "Source IP", "Destination IP", "Destination Port", "Source Port", "Protocol", "Label"]
    ]
    df_CICIDS_csv.rename(
        columns={
            "Timestamp": "stime",
            "Source IP": "srcip",
    ...
    df_CICIDS_csv["stime"] = df_CICIDS_csv["stime"].apply(
        lambda x: (datetime.strptime(x, "%d/%m/%Y %H:%M"))
        if x.count(":") == 1
        else (datetime.strptime(x, "%d/%m/%Y %H:%M:%S"))
    )
    df_CICIDS_csv["stime"] = df_CICIDS_csv["stime"].apply(
        lambda x: int((datetime(x.year, x.month, x.day, (x.hour + 12), x.minute, x.second)).timestamp())
        if (x.hour >= 1) & (x.hour <= 7)
        else int((datetime(x.year, x.month, x.day, x.hour, x.minute, x.second)).timestamp())
    )
    ...
    for pcap_file in pcap_csv:
        ...
        if file_num == 1:
            ...
            combine.drop(
                combine[
                    (combine.stime >= 1499177940) & (combine.stime <= 1499181660) & (combine.label == "BENIGN")
                ].index,
                inplace=True,
            )
            combine.drop(
                combine[
                    (combine.stime >= 1499194800) & (combine.stime <= 1499198400) & (combine.label == "BENIGN")
                ].index,
                inplace=True,
            )
            ...
            ltime = int(
                1499286720
            )  ## '05/07/2017 15:32:00' (Since this time doesnt exist in PCAP, therefore mitigating its issue)
            df_pcap_csv.drop(columns=["frame_num", "stime", "ltime", "protocol_s"], inplace=True)
            df_red = df_CICIDS_csv[(df_CICIDS_csv["stime"] >= stime) & (df_CICIDS_csv["stime"] <= ltime)]
            combine = df_pcap_csv.merge(
                df_red,
            ...
            combine.drop(
                combine[
                    (combine.stime >= 1499266020) & (combine.stime <= 1499267400) & (combine.label == "BENIGN")
                ].index,
                inplace=True,
            )
            combine.drop(
                combine[
                    (combine.stime >= 1499267640) & (combine.stime <= 1499268900) & (combine.label == "BENIGN")
                ].index,
                inplace=True,
            )
            combine.drop(
                combine[
                    (combine.stime >= 1499269380) & (combine.stime <= 1499270400) & (combine.label == "BENIGN")
                ].index,
                inplace=True,
            )
            combine.drop(
                combine[
                    (combine.stime >= 1499271000) & (combine.stime <= 1499271780) & (combine.label == "BENIGN")
                ].index,
                inplace=True,
            )
            combine.drop(
                combine[
                    (combine.stime >= 1499285520) & (combine.stime <= 1499286720) & (combine.label == "BENIGN")
                ].index,
                inplace=True,
            )
            ...
            combine.drop(
                combine[
                    (combine.stime >= 1499371200) & (combine.stime <= 1499373900) & (combine.label == "BENIGN")
                ].index,
                inplace=True,
            )
            combine.drop(
                combine[
                    (combine.stime >= 1499369400) & (combine.stime <= 1499369700) & (combine.label == "BENIGN")
                ].index,
                inplace=True,
            )
            combine.drop(
                combine[
                    (combine.stime >= 1499355600) & (combine.stime <= 1499355720) & (combine.label == "BENIGN")
                ].index,
                inplace=True,
            )
            combine.drop(
                combine[
                    (combine.stime >= 1499354100) & (combine.stime <= 1499355300) & (combine.label == "BENIGN")
                ].index,
                inplace=True,
            )
            combine.drop(
                combine[
                    (combine.stime >= 1499350800) & (combine.stime <= 1499353200) & (combine.label == "BENIGN")
                ].index,
                inplace=True,
            )
            ...
            ltime = int(
                1499462160
            )  ## 07/07/2017 16:16:00 (Since this time doesnt exist in PCAP, therefore mitigating its issue)
            df_pcap_csv.drop(columns=["frame_num", "stime", "ltime", "protocol_s"], inplace=True)
            df_red = df_CICIDS_csv[(df_CICIDS_csv["stime"] >= stime) & (df_CICIDS_csv["stime"] <= ltime)]
            combine = df_pcap_csv.merge(
                df_red,
            ...
            combine.drop(
                combine[
                    (combine.stime >= 1499460960) & (combine.stime <= 1499462160) & (combine.label == "BENIGN")
                ].index,
                inplace=True,
            )
            combine.drop(
                combine[
                    (combine.stime >= 1499453700) & (combine.stime <= 1499459400) & (combine.label == "BENIGN")
                ].index,
                inplace=True,
            )
            combine.drop(
                combine[
                    (combine.stime >= 1499439720) & (combine.stime <= 1499443320) & (combine.label == "BENIGN")
                ].index,
                inplace=True,
            )
